...
export default function Final({gameState, setGameState, score, charactersLeft, setCharactersLeft, setScore, setTimeLeft, endRoundChars, setEndRoundChars, record, setRecord, recordBeat, setRecordBeat}) {
  ...
  function handleKeyPress(event) {
    ...
    return (
        <div className="flex flex-col justify-center items-center h-screen">
            <h1 className="p-2 m-2 text-6xl font-black text-purple-100"> FINAL SCORE </h1>
            <h2 className="p-2 m-2 text-2xl font-black text-purple-900 bg-purple-200"> You scored: {score}</h2>
            {recordBeat && <h2 className="p-2 m-2 text-2xl font-black bg-green-700 text-green-100"> THAT'S A NEW HIGH SCORE! </h2> }
            <h4 className="p-2 m-1 text-2xl font-bold italic text-purple-100"> The AI says... </h4>
            <h2 className="p-2 m-2 text-2xl font-black text-purple-100 w-1/2 flex-wrap text-center"> {feedback} </h2>
            <button onClick={startGame} ref={newestInputRef} onKeyPress={handleKeyPress} className="px-4 py-2 m-4 text-purple-900 bg-purple-200 hover:bg-purple-300 font-black shadow-md rounded"> PLAY AGAIN </button>
        </div>
    )
}
